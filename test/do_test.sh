...
echo | while read x; do
    ...
    if [ ${separate} -ne 0 ]; then
        echo ${DMD} ${MODEL} -I${input_dir} ${r_args} $x -od${output_dir} -of${test_app_dmd} ${extra_compile_args} ${all_sources[*]} >> ${output_file}
             ${DMD} ${MODEL} -I${input_dir} ${r_args} $x -od${output_dir} -of${test_app_dmd} ${extra_compile_args} ${all_sources[*]} >> ${output_file} 2>&1
        ...
        for file in ${all_sources[*]}; do
            echo ${DMD} ${MODEL} -I${input_dir} ${r_args} $x -od${output_dir} -c $file >> ${output_file}
                 ${DMD} ${MODEL} -I${input_dir} ${r_args} $x -od${output_dir} -c $file >> ${output_file} 2>&1
            if [ $? -eq ${expect_compile_rc} -o $? -gt 125 ]; then
                cat ${output_file}
                rm -f ${output_file}
                exit 1
        ...
        if [ "${input_dir}" = "runnable" ]; then
            echo ${DMD} ${MODEL} -od${output_dir} -of${test_app_dmd} ${all_os[*]} >> ${output_file}
                 ${DMD} ${MODEL} -od${output_dir} -of${test_app_dmd} ${all_os[*]} >> ${output_file} 2>&1
            if [ $? -eq ${expect_compile_rc} -o $? -gt 125 ]; then
                cat ${output_file}
                rm -f ${output_file}
                exit 1
